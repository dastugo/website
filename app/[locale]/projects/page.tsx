...
import type { SanityProject } from '@/components/sections/ProjectsSlider'
...
export default async function ProjectsPage({ params }: Props) {
  const { locale } = await params
  const t = await getTranslations('projects')
  const projects: SanityProject[] = await client.fetch(allProjectsQuery(locale))
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project) => (
              <article key={project.slug}
                ...
                <Link href={`/${locale}/project/${project.slug}`} className="block relative aspect-video overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 pt-3 border-t border-border">
                    ...
                    {project.githubUrl && (
                      <a href={project.githubUrl} target="_blank" rel="noopener noreferrer"
                        ...
                        aria-label={t('viewGithub')}><Github size={15} /></a>
                    )}
                    {project.liveUrl && (
                      <a href={project.liveUrl} target="_blank" rel="noopener noreferrer"
                        className="p-1.5 rounded-md hover:bg-muted transition-colors text-muted-foreground hover:text-foreground"
                        aria-label={t('liveDemo')}><ExternalLink size={15} /></a>
                    )}
                  </div>
                </div>
              </article>
